...
### 참고 노트북 — Weights & Biases

`code/3.1.2.0.ConvNet_WnB.ipynb` 는 같은 MLP/CNN 학습에 Weights & Biases(W&B) logger 를 붙여 학습 곡선과 gradient·parameter histogram 을 기록하고, best 모델 checkpoint 를 W&B 에 저장했다가 다시 load 해 보는 참고 노트북이다. 본 절의 실습 범위 밖이므로 실험 관리 도구에 관심이 있으면 따로 열어 본다.
